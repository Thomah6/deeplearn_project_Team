...
<template>
  <div class="antialiased mx-auto p-4 dark:text-white">
    ...
    <form @submit.prevent="addComment" class="mb-6">
      <!-- Tiptap Toolbar -->
      <div v-if="editor" class="flex items-center justify-between px-3 py-2 border-t border-x border-gray-200 dark:border-gray-600 rounded-t-lg">
        <div class="flex flex-wrap items-center divide-gray-200 sm:divide-x dark:divide-gray-600">
          <div class="flex items-center space-x-1 sm:pr-4">
            <button @click="editor.chain().focus().toggleBold().run()" type="button" class="p-2 text-gray-500 rounded cursor-pointer" :class="{ 'bg-gray-200 dark:bg-gray-700': editor.isActive('bold') }"><strong>B</strong></button>
            <button @click="editor.chain().focus().toggleItalic().run()" type="button" class="p-2 text-gray-500 rounded cursor-pointer" :class="{ 'bg-gray-200 dark:bg-gray-700': editor.isActive('italic') }"><em>I</em></button>
            <button @click="editor.chain().focus().toggleCode().run()" type="button" class="p-2 text-gray-500 rounded cursor-pointer" :class="{ 'bg-gray-200 dark:bg-gray-700': editor.isActive('code') }"><code>&lt;/&gt;</code></button>
            <button @click="editor.chain().focus().toggleBlockquote().run()" type="button" class="p-2 text-gray-500 rounded cursor-pointer" :class="{ 'bg-gray-200 dark:bg-gray-700': editor.isActive('blockquote') }">“ ”</button>
          </div>
        </div>
      </div>

      <div
        class="py-2 px-4 bg-white rounded-b-lg border border-gray-200 dark:bg-gray-800 dark:border-gray-700"
      >
      ...
      >
        ...
        <div v-if="editingCommentId === comment.id">
          <textarea
            v-model="editingText"
            rows="3"
            class="px-0 w-full text-sm text-gray-900 border-0 focus:ring-0 focus:outline-none dark:text-white dark:placeholder-gray-400 dark:bg-gray-800 mb-2"
          ></textarea>
        ...
        <div v-else>
          <div class="prose prose-sm max-w-none text-gray-500 dark:text-gray-400 dark:prose-invert" v-html="comment.text"></div>
          <div v-if="currentUser && currentUser.id === comment.userId" class="flex items-center mt-4 space-x-4">
            <button @click="startEditing(comment)" class="flex items-center text-sm text-gray-500 hover:underline dark:text-gray-400 font-medium">
              Modifier
            </button>
            <button @click="deleteComment(comment.id)" class="flex items-center text-sm text-red-500 hover:underline dark:text-red-400 font-medium">
              Supprimer
            </button>
          </div>
        </div>
