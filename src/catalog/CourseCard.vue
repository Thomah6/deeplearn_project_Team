    ...
    <div class="relative  inline-block text-left">
      ...
      >
        <ul class="py-1 text-sm text-gray-700 dark:text-gray-200">
          <li><span @click="()=>{filterByLevel('Débutant')}" class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white cursor-pointer">Débutant</span></li>
          <li><span @click="()=>{filterByLevel('Intermédiaire')}" class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white cursor-pointer">Intermédiaire</span></li>
          <li><span @click="()=>{filterByLevel('Avancé')}" class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white cursor-pointer">Avancé</span></li>
        </ul>
      </div>
    </div>
...
<article class="grid w-full mt-16 justify-center sm:grid-cols-2 xl:grid-cols-3 gap-6 sm:gap-8 lg:gap-10 px-4 sm:px-6 lg:px-8">
      ...
      <div class="h-full  justify-center  mx-auto hover:shadow-2xl rounded-2xl cursor-pointer transition ease-in duration-500  transform ">
          ...
          <div class="relative flex reltive  w-full max-w-[26rem] flex-col h-full rounded-xl bg-white dark:bg-gray-800 bg-clip-border text-gray-700 dark:text-white/80 shadow-lg">
              ...
              <div class="p-6 flex-grow">
                  ...
                  <p class="block font-sans text-base antialiased font-light leading-relaxed text-gray-700 dark:text-white/80">
 ...
 <span
          ...
          <span class="absolute top-0  right-0  py-2 px-8 rounded-bl-xl rounded-tr-xl  dark:text-white/80 font-semibold bg-sky-100 border border-sky-200 dark:bg-gray-700">{{ cours.category }}</span>
            </div>
      </div>
